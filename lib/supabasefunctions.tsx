  ...
  try {
    ...
    if (allSongs && allSongs.length > 0) {
      ...
      if (insertError) {
        ...
        await supabase.from("user_playlists").delete().eq("id", newPlaylist.id);
    ...
    alert(
      `Playlist "${playlist.name}" saved to your library with ${allSongs?.length || 0} songs!`,
    );
    return true;
  } catch (error) {
...
export const saveToListeningHistory = async (song) => {
  try {
    ...
    const { error } = await supabase.from("user_listening_history").insert({
      user_id: user.id,
      song_id: song.id,
...
export const getUserSubscription = async () => {
  try {
    ...
    console.log("user in function:", user?.id, "authError:", authError);
    if (!user) return null;
    ...
    console.log("subscriptions raw:", subscriptions, "error:", error);

    if (error || !subscriptions || subscriptions.length === 0) return null;

    const subscription = subscriptions[0]; // take the most recent

    // expiry check
    if (subscription.current_period_end && subscription.plan_type !== "free") {
      const now = new Date();
...
export const deductUserCredits = async (
  ...
  try {
    ...
    if (subscription.credits_remaining < creditsNeeded) {
      throw new Error(
        `Insufficient credits. You need ${creditsNeeded} credit${creditsNeeded > 1 ? "s" : ""} to download this ${downloadType}, but you only have ${subscription.credits_remaining}.`,
      );
    }
